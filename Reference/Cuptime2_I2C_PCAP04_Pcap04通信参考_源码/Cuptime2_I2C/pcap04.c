#include<iostm8l151k4.h>
#include <stdint.h>
#include <stdio.h>
#include"usart1.h"
#include"pcap04.h"
#include"i2c.h"

// Standard Device Address 0 | 1 | 0 | 1 | 0 | A1 | A0 | R/W
#define PCAP04_I2C  0x28
// Opcode 0x40 + Result Register RES01 0x04
#define REG         0x44
// OPCODES
#define WR_MEM      0xA0
#define WR_CONFIG   0xA3
#define POR         0x88
#define INITIALIZE  0x8A
#define CDC_START   0x8C
#define RDC_START   0x8E
#define NV_STORE    0x96
#define TEST        0x7e

unsigned char I2c_Buf;
unsigned char RES[32];

// Standard Firmware is 1024 Bytes long (0..1024)
unsigned char PCAP04_StandardFirmware[] = {

};


//REG7= AV Rate
//REG09,REG10 = Conversion Time
//REG13 = Opcode Triggered
//REG15 = C_Fake
//REG34 = AUTOSTART

unsigned char PCAP04_Config[] = {
 0x1D, 0x00, 0x58, 0x10,  
 0x10, 0x00, 0x0F, 0x20,  
 0x00, 0xD0, 0x07, 0x00,  
 0x00, 0x08, 0xFF, 0x03,  
 0x00, 0x24, 0x00, 0x00,  
 0x00, 0x01, 0x50, 0x30,  
 0x73, 0x04, 0x50, 0x08,  
 0x5A, 0x00, 0x82, 0x08,  
 0x08, 0x00, 0x47, 0x40,  
 0x00, 0x00, 0x00, 0x71,  
 0x00, 0x00, 0x08, 0x00,  
 0x00, 0x00, 0x00, 0x01,  
 0x00, 0x00, 0x00, 0x00
};


unsigned char readdata;
uint32_t ratioPCAP04;


void PCap04_Test()
{
  
  readdata = IIC_Read_Pcap04(PCAP04_I2C, TEST);

}

void PCap04_PowerON_RESET()
{
  IIC_Write_Pcap04(PCAP04_I2C, POR);
}

void PCap04_Init()
{
  IIC_Write_Pcap04(PCAP04_I2C, INITIALIZE);
}

void PCap04_writeFirmware()
{
 unsigned char Opcode = 0xA0;
// uint16_t address = 0x0000;

 //uint8_t op_addr1 = (uint8_t)(opcode | address >>8);
// uint8_t op_addr2 = (uint8_t)(address);
 
 IIC_Write(PCAP04_I2C , Opcode, 0x00, PCAP04_StandardFirmware , 1024);
 
}

void PCap04_writeConfig()
{
  unsigned char Opcode = 0xA3;
  
  IIC_Write(PCAP04_I2C , Opcode, 0xc0, PCAP04_Config , 52);
}

void PCap04_CDCStart()
{
  IIC_Write_Pcap04(PCAP04_I2C, CDC_START);
}

void PCap04_ReadResult()
{
  unsigned char i; 
  IIC_Read(PCAP04_I2C, 0x40 , RES, 32);
    for (i=0; i<32; i++)
    {
      printf("\r\n RES[%d]: %d", i, RES[i]);
    }
}