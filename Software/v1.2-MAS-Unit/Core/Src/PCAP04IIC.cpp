...
/** 移植
 * 待修改：
 * 1.串口输出，原工程使用的式arduino库的Serial类，需要改为STM32 USB的虚拟串口打印
 *
 */

#define PCAP_DEBUG 0
#define PCAP_DOWNLOAD 1
...
bool PCAP04IIC::test_connection()
{
    ...
    uint8_t recval = 0;
    uint8_t sendcode;
    usb_printf("I2C read test start, address: 0x%x \r\n", i2c_address>>1);
...
*/
    sendcode = (test_read.opcode >> 8) & 0xFF;
    if(HAL_OK == HAL_I2C_Master_Transmit(&hi2c1, i2c_address, &sendcode, 1, 0xffff)){
        HAL_I2C_Master_Receive(&hi2c1, i2c_address, &recval, 1, 0xffff);
    }

    if (recval == 0x11)
    {
        usb_printf("I2C read test:success read 0x%x\r\n", recval);
//    Serial.println("I2C read test:success");
        testres = true;
    }
    else
    {
        usb_printf("I2C read test:failed -> ");
        usb_printf(" %c ", recval);
        usb_printf("expected: 0x11\r\n");
//    Serial.print("I2C read test:failed -> ");Serial.print(recval); Serial.println(" expected: 0x11");
        testres = false;
    }
    usb_printf("I2C read test end\r\n");
...
{
    static unsigned char recval = 0;
    uint8_t sendData0 = (uint8_t)((data >> 16) & 0xFF);
    uint8_t sendData1 = (uint8_t)((data >> 8) & 0xFF);
    uint8_t sendData2 = (uint8_t)(data & 0xFF);
    uint8_t array_senddata[3] = {sendData0, sendData1, sendData2};
 ...
 */
    ...
    if(sendData0 > 0x7f){ //If the MSB is 1, then only transmit
        HAL_I2C_Master_Transmit(&hi2c1, i2c_address, (uint8_t*)array_senddata, 3, 0xffff);
        HAL_Delay(1);
        return recval;
    }
    else{
        HAL_I2C_Master_Transmit(&hi2c1, i2c_address, (uint8_t*)array_senddata, 2, 0xffff);
        HAL_I2C_Master_Receive(&hi2c1, i2c_address, &recval, 1, 0xffff);
    }
  
  //If the MSB is 0, then receive as well, but don't send the last byte
    HAL_Delay(1);
    return recval;
};
...
//  Wire.beginTransmission(i2c_address);
    if (transmittedData[0] > 63){
        HAL_I2C_Master_Transmit(&hi2c1, i2c_address, &transmittedData[0], 1, 0xffff);
    //    Wire.write(transmittedData[0]); //Reading result, only using the first byte
    }
    else{
        HAL_I2C_Master_Transmit(&hi2c1, i2c_address, (uint8_t *)transmittedData, 2, 0xffff);
...
unsigned char PCAP04IIC::i2c_transmit(unsigned char data)
{
  ...
  // Serial.print("Data: ");Serial.println(data, BIN);
    ...
    HAL_I2C_Master_Transmit(&hi2c1, i2c_address, &data, 1, 0xffff);
//  Wire.beginTransmission(i2c_address);
//  Wire.write(data);
...
//    usb_printf("Por has been reset\r\n");
//    HAL_Delay(100);

//    write_config(47,0); // STOP DSP

//    usb_printf("Config has been written\r\n");
//    HAL_Delay(100);

    #if PCAP_DOWNLOAD
        writeall_nvram();
        usb_printf("nvram has been written\r\n");
    #endif

    #if PCAP_DEBUG
        validate_nvram();
        readall_nvram();
        usb_printf("nvram has been read, RUNBIT: on pcap nvram - %x, RUNBIT: on teensy nvram - %x\r\n", pcap_nvram_mirror.CFG.CFG47.REGVAL, pcap_nvram.CFG.CFG47.REGVAL);
        readall_config();
    #endif

//    usb_printf("RUNBIT: on pcap nvram valid - %x \r\n", pcap_nvram_mirror.CFG.CFG47.REGVAL);

//    send_command(INITIALIZE_OP);

//    usb_printf("Initialize op has been sent\r\n");

//    send_command(CDC_START);
 
    initialized = true;

//    usb_printf("valid config\r\n");
  
//    print_config();


//    usb_printf("pcap initialized \r\n");
...
//  Serial.println("Config has been written");
  ...
  readall_nvram();
    ...
    usb_printf("nvram has been read \r\n");
...
//  Serial.println("nvram has been written");
  ...
  validate_nvram();
    ...
    usb_printf("RUNBIT: on pcap nvram valid %c", pcap_nvram_mirror.CFG.CFG47.REGVAL);
...
*/
  ...
  {
      usb_printf("write config to address %c - skipped (CHARGE_PUMP)\r\n", addr);
  }
  else
  {
      usb_printf("write config address not in range [0-63] %c", addr);
  }
}
...
void PCAP04IIC::read_nvram(unsigned short addr)
{
  ...
  if ((addr >= 0) && (addr < PCAP_NVRAM_SIZE))
  {
    ...
    usb_printf("read address 0x%x:(0x%x)\r\n", rd_mem.nvram.addr, rd_mem.nvram.data);
...
*/      
  ...
  {
      usb_printf("read nvram address not in range [0-1023]: %c", addr);
...
void PCAP04IIC::read_config(unsigned char addr)
{
  ...
  if ((addr >= 0) && (addr < PCAP_NVRAM_CFG_SIZE))
  {
    ...
    usb_printf("read config address 0x%x: (0x%x\r\n)",rd_config.config.addr, rd_config.config.data);
  ...
  {
      usb_printf("read config address not in range [0-63]: %x", addr);
...
//  SPI.beginTransaction(pcap_spi_settings);
  ...
  {
      usb_printf("read result address not in range [0-34] : %c", addr);
  ...
  for (int i = 0; i < PCAP_NVRAM_SIZE; i++){
    ...
    if (i < PCAP_NVRAM_MAX_INDEX_FW)
    {
      ...
      if (pcap_nvram.FW.data[s] <= 0x0F){
          usb_printf("0%c ", pcap_nvram.FW.data[s]);
//        Serial.print("0");Serial.print(pcap_nvram.FW.data[s],HEX);Serial.print(" ");
      }else{
          usb_printf("%c ", pcap_nvram.FW.data[s]);
//        Serial.print(pcap_nvram.FW.data[s],HEX);Serial.print(" ");
    ...
    else if (i >= PCAP_NVRAM_MAX_INDEX_FW && i < PCAP_NVRAM_MAX_INDEX_FW_CAL0)
    {
      ...
      if (pcap_nvram.FW_CAL0.data[s] <= 0x0F){
          usb_printf("0%c ", pcap_nvram.FW_CAL0.data[s]);
//        Serial.print("0");Serial.print(pcap_nvram.FW_CAL0.data[s],HEX);Serial.print(" ");
      }else{
          usb_printf("%c ", pcap_nvram.FW_CAL0.data[s]);
//        Serial.print(pcap_nvram.FW_CAL0.data[s],HEX);Serial.print(" ");
    ...
    else if (i >= PCAP_NVRAM_MAX_INDEX_FW_CAL0 && i < PCAP_NVRAM_MAX_INDEX_FW_CAL1)
    {
      ...
      if (pcap_nvram.FW_CAL1.data[s] <= 0x0F){
          usb_printf("0%c ", pcap_nvram.FW_CAL1.data[s]);
//        Serial.print("0");Serial.print(pcap_nvram.FW_CAL1.data[s],HEX);Serial.print(" ");
      }else{
          usb_printf("%c ", pcap_nvram.FW_CAL1.data[s]);
...
void PCAP04IIC::write_nvram(unsigned short addr){
  ...
  {
      usb_printf("write nvram address not in range [0-1023]:%c", addr);
...
pcap_status_t* PCAP04IIC::get_status(bool print_status){
  ...
  if (print_status){
      usb_printf("%x %x %x \r\n", pcap_results_regs.STATUS0.REGVAL, pcap_results_regs.STATUS1.REGVAL, pcap_results_regs.STATUS2.REGVAL);
//    Serial.print(pcap_results_regs.STATUS0.REGVAL, BIN);Serial.print(" ");
//    Serial.print(pcap_results_regs.STATUS1.REGVAL, BIN);Serial.print(" ");
...
void PCAP04IIC::validate_nvram(){
  ...
  for (size_t addr = 0; addr < PCAP_NVRAM_SIZE; addr++)
  {
    ...
    if (*nvram_p != *nvram_mirror_p){
        usb_printf("nvram different at address : 0x%x with values (mcu, pcap) :(0x%x,0x%x).\r\n", addr, *nvram_p, *nvram_mirror_p);
